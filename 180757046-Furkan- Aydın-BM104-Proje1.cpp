...
DERSÝN KODU: BIM-104-50-50-D
AKADEMÝK DÖNEM: 4
PROJE NO:1
ÖÐRETÝM ÜYESÝ ADI: GÜLÞAH TÜMÜKLÜ ÖZYER
*/
...
void mayinyerlestirme(char d[][5],int boyut);
void matrisibasma(char d[][5],int boyut);
void tekrarsorusu();
...
{	
	...
	mayinyerlestirme(oyunMat,6);// Matrisi ve içine rastgele mayinlarý koyduðum fonksiyonu programýn baþýnda çaðýrdým. Fonksiyona parametre olarak dizimin ismini
	//ve dizimin uzunluðunu yolladým.
	...
	if(satir>5)
	{
		...
		cout<<endl;
		matrisibasma(oyunMat,6);//oyunun sonunda kullanýcýnýn yolunu ve patlayan mayýný matris halinde ekrana bastýrdým.
		cout<<endl;
		tekrarsorusu();//kullanýcýya tekrar oynayýp oynamak istemediði sorusunu soran fonksiyonu çaðýrdým.
			...
			else //Hayýr seçeneðini seçtiði taktirde programý sonlandýrdým.
			{	
	...
	if(oyunMat[5][0]!= 'P' && oyunMat[5][1]!= 'P' && oyunMat[5][2]!= 'P' && oyunMat[5][3]!= 'P' && oyunMat[5][4]!= 'P')	
	{
		...
		cout<<endl;
		matrisibasma(oyunMat,6);//Matrisi bastýran fonksiyonumu çaðýrdým.
		cout<<endl;
...
void matrisibasma(char d[][5],int boyut) // Matrisi bastýrdýðým fonksiyon.
{
...
//Bu fonksiyona oyunMat isimli 2 boyutlu dizimin parametrelerini gönderdim. Yeni dizime d ismini verdim.
void mayinyerlestirme(char d[][5], int boyut)//Mayinlarý rastgele yerleþtirdiðim fonksiyon.
{	
